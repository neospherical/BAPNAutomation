...
class CustomHelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
      ...
      help_embed.add_field(name="```.leaderboard```", value="*Use this to display the current Official Leaderboard Rankings*", inline=True)
      ...
      help_embed.add_field(name="```.addme```", value="*Use this to request to participate on the Official Leaderboard Rankings*", inline=True)
...
async def ayuda(ctx):
  ...
  help_embed.add_field(name="```.rankCheck (Nombre de usuario)```", value="*Use esto para ver que posición ocupa alguien en la clasificatoria*", inline=True)
...
async def challenge(ctx, your_username, opponent_username):
    ...
    if challengeChecks():
        ...
        try:
          ...
          await dm.send(f"You have been challenged by {ctx.message.author.mention} ({your_username})! Please consider contacting them in order to schedule your fight.")
        except:
          return
    else:
...
async def addme(ctx):
  ...
  msg = await dm.send(embed=introEmbed, components=[components])
  await discordClient.wait_for("button_click", check=buttonCheck)
  await msg.edit(embed=introEmbed, components=[])
  ...
  try:
    ...
    await dm.send("An error has occurred. The bot detected an invalid input. Please say ``.addme`` and try again with a valid AVK.")
    return
  if res < 50.0:
    await dm.send("You do not have enough avk to rejoin the leaderboard. Please come back when you have ganed at least 50 avk.")
    return
  elif 50.0 <= res < 100.0:
    startingSR = 4700
  elif res >= 100.0:
  ...
  avkProofEmbed.set_image(url="https://cdn.discordapp.com/attachments/718291713738670151/933913994337607690/azlinsavk.png")
  ...
  msg = await dm.send(embed=weaponEmbed, components=[components])
  res = await discordClient.wait_for("button_click", check=buttonCheck)
  await msg.edit(embed=weaponEmbed, components=[])
  weapon = res.component.custom_id
  ...
  confirmationPrompt.add_field(name="__AVK__", value=f"{avk} (Your starting Skill Rating will be {startingSR})", inline=False)
  ...
  confirmationPrompt.set_author(name=str(ctx.message.author), icon_url=ctx.message.author.avatar_url)
  components = [
    Button(style=ButtonStyle.green, label="Send", custom_id="yes"),
    Button(style=ButtonStyle.red, label="Dont Send", custom_id="no")
  ]
  msg = await dm.send(embed=confirmationPrompt, components=[components])
  res = await discordClient.wait_for("button_click", check=buttonCheck)
  await msg.edit(embed=confirmationPrompt, components=[])
  ...
  msg = await applicationChannel.send(embed=confirmationPrompt, components=[newComponents])
  ...
  if not alreadyHasSheet:
    newWorksheet = None
    if weapon == "Katana":
      newWorksheet = spreadsheet.duplicate_sheet(1511070903, insert_sheet_index=0, new_sheet_name=rUser)
    elif weapon == "Heavy":
      newWorksheet = spreadsheet.duplicate_sheet(2115675779, insert_sheet_index=0, new_sheet_name=rUser)
    worksheets = spreadsheet.worksheets()
    sheetsToSort = []
    for worksheet in worksheets:
